...
def show(duel, user, room_number, ask, decks, graves, hands):
    ...
    if duel.in_cost is True:
        ...
        if cost_det.cost_val == 26:
            ...
            return_value["prompt"] = duelobj.write_prompt(prompt,user)
            return HttpResponse(json.dumps(return_value))
        elif cost_det.cost_val == 76:
            return_value = {}
            chain_det_ary = duelobj.chain_det_trigger
            chain_det = chain_det_ary[str(duelobj.duel.chain )]
            ...
            return_value["user"] = user
            return_value["monster"] = duelobj.get_fusion_material(cost_det,user, trigger,1,1,1)
            return_value["field_info"] = field
            return HttpResponse(json.dumps(return_value))
        ...
        if (
            ...
            or monster_effect_det2.monster_effect_val == 81 
            or monster_effect_det2.monster_effect_val == 82 
        ):
            if duel.user_turn == effect_user:
                ask_org = 1
            else:
            ...
            ask_org = 0
        if monster_effect_det2.monster_effect_val == 81:
            step = monster_effect_text_org["monster"][0]["step"]
        else:
            step = 0
    tmp_val = {}
    return_val = []
                            ...
                            for y in range(field_size.field_y):
                                ...
                                if field[x][y]["det"] is not None and user != field[x][y]["mine_or_other"] and int(duelobj.check_change_val(
                                    ...
                                    if duelobj.config.sort is True:
                                        field = duelobj.sortField(field,y)
                                    field[x][y]["hide"] = True
                                flag_field_place = False
                                if duelobj.field_free is False:
                                    kind = field[x][y]["kind"]
...
def choose_trigger(duel, user, room_number, ask, decks, graves, hands):
     ...
     order = config.order
     trigger_waiting_json = duelobj.trigger_waiting
     tmp_return  = {}
     return_trigger = []
     i = 0
     force = False
