...
class tmp_class:
    pass
def field_list_view_x(request):
    if not request.user.is_staff:
        return HttpResponse("error")
    form = FieldXForm(request.POST)
    if form.is_valid():
        field = {}
        field["y"] = request.POST["y"]
        field["no_clear"] = form.cleaned_data.get('no_clear')
        field["box"] = form.cleaned_data.get('box')
        field["color"] = form.cleaned_data.get('color')
        field["kind"] = form.cleaned_data.get('kind')
        field["sentence"] = form.cleaned_data.get('sentence')
        field["mine_or_other"] = form.cleaned_data.get('mine_or_other')
        Field.objects.filter(y=request.POST["y"]).update(**field)
        return HttpResponseRedirect(reverse("field_list_view"))
    y = request.GET["y"]
    form.y = forms.BooleanField(label="y",required=True,widget=forms.HiddenInput,initial=y)
    context = {'form':form,'y':y}
    return render(request, "admin/tcgcreator/field_list_x.html",context)
...
def get_equation(req):
    ...
    result += (
            'step<input type="number" style="width:50px" id="step_'
        + c
        + '" value="1" '
        + ">"
    )
...
def makedecktext(req):
    ...
    monster_variables = MonsterVariables.objects.order_by("-priority").select_related(
...
def download_card(request):
    ...
    config = Config.objects.first();
    if config.csv_encoding == 0:
    	response = HttpResponse(content_type='text/csv;charset=Shift-JIS')
    elif config.csv_encoding == 1:
    	response = HttpResponse(content_type='text/csv;charset=EUC_JP')
    elif config.csv_encoding == 2:
    	response = HttpResponse(content_type='text/csv;charset=UTF-8')
    response['Content-Disposition'] = 'attachment; filename="card_data.csv"'
    writer = csv.writer(response)
    monsters = Monster.objects.all();
    data_label = []
    monster_variables = MonsterVariables.objects.order_by("-priority")
    data_label.append("画像")
    data_label.append("カード名")
    data_label.append("カードテキスト")
    data_label.append("枚数制限")
    data_label.append("デッキ")
    monster_variable_kind_data = {}
    for monster_variable in monster_variables:
        if monster_variable.monster_variable_name == "show":
            continue
        if monster_variable.monster_variable_name == "rotate":
            continue
        monster_variable_kind_data[str(monster_variable.id)] = (monster_variable.monster_variable_kind_id.monster_variable_sentence)
        data_label.append(monster_variable.monster_variable_name)
    writer.writerow( data_label)
    for monster in monsters:
        data = []
        data.append(monster.img)
        data.append(monster.monster_name)
        data.append(monster.monster_sentence)
        data.append(monster.monster_limit)
        deck_data = monster.monster_deck.split("_")
        deck_name = ""
        for deck in deck_data:
            tmp = Deck.objects.get(id=deck) 
            deck_name += tmp.deck_name + "_"
        if deck_name != "":
            deck_name = deck_name[:-1]
        data.append(deck_name)
        for monster_variable in monster_variables:
            if monster_variable.monster_variable_name == "show":
                continue
            if monster_variable.monster_variable_name == "rotate":
                continue
            monster_item = MonsterItem.objects.filter(monster_id=monster,monster_variables_id__id=monster_variable.id).first()
            if monster_variable.monster_variable_kind_id.id == 1:
                text = monster_item.monster_item_text
            else:
                text = ""
                item_sentence = monster_variable_kind_data[str(monster_variable.id)].split("|")
                for val_data in monster_item.monster_item_text.split("_"):
                    if text == "":
                       text = item_sentence[int(val_data)-1]
                    else:
                       text += "_"+item_sentence[int(val_data)-1]
            data.append(text)
...
def save_unit(request):
    ...
    config = Config.objects.get();
    if config.csv_encoding == 0:
        data_file = open("./data.csv", mode="r", encoding="shift_jis")
    elif config.csv_encoding == 1:
        data_file = open("./data.csv", mode="r", encoding="euc_jp")
    elif config.csv_encoding == 2:
        data_file = open("./data.csv", mode="r", encoding="utf-8")
    ...
    i=-1
    i+=1
    monster_variable = MonsterVariables.objects.get(monster_variable_name = "show");
    obj = {}
    obj["num"] = i
    ...
    i+=1
    monster_variable = MonsterVariables.objects.get(monster_variable_name = "rotate");
    obj = {}
    obj["num"] = i
    obj["val_id"] = monster_variable
    obj["id"] = 99999
    ...
    for data in data3 :
        ...
        if Monster.objects.filter(monster_name=datas[1]).exists():
               monster = Monster.objects.filter(monster_name=datas[1]).first()
               update_flag = True
        else:
               monster = Monster()
               update_flag = False
        ...
        monster.monster_limit = datas[3]
        deck_datas = datas[4]
        deck_datas = deck_datas.split("_")
        monster_deck = ""
        for deck_data in deck_datas:
            deck = Deck.objects.filter(deck_name=deck_data).first()
            if monster_deck == "":
                monster_deck = str(deck.id)
            else:
                monster_deck += "_"+str(deck.id)
        monster.monster_deck = monster_deck
        monster.save()
        i = 0
        for data_obj in data_label_det2:
                if(data_obj["id"] == 99999):
                    text = "0"
                elif(data_obj["id"] == 1):
                    text = datas[data_obj["num"]-2]
                    if text == "":
                        text = "0"
                else:
                    data_text = datas[data_obj["num"]-2]
                    if data_text != "":
                        item_sentence = data_obj["sentence"].split("|")
                        text = ""
                        for data_num in datas[data_obj["num"]-2].split("_"):
                            if text == "":
                                text = str(item_sentence.index(data_num)+1)
                            else:
                                text += "_"+str(item_sentence.index(data_num)+1)
                    else:
                        text = "1"
                if text == "":
                    text = "0"
                if update_flag is True:
                       monster_item = MonsterItem.objects.filter(monster_id=monster,monster_variables_id=data_obj["val_id"]).first()
                	   ...
                	   monster_item_text=text
                           ...
                           monster_item.monster_item_text = text
                else:
                        monster_item = MonsterItem(
                	    monster_id=monster,
                	    monster_variables_id=data_obj["val_id"],
