...
#    chain_user = duelobj.get_current_chain_user()
    ...
    if (
            ...
            and len(duelobj.trigger_waiting)
        ...
        duelobj.invoke_trigger_waiting(duelobj.trigger_waiting, tmp_priority)
        duelobj.update = True
        flag = True
        duelobj.check_eternal_effect(
            decks, graves, hands, duel.phase, duel.user_turn, user, other_user
        ...
        if duel.in_cost >= 1 and duelobj.in_execute is False and duel.appoint == user:
            ...
            while duel.winner == 0 and duel.winner_ai == 0:
                        ...
                        if choices2[0] is not None and duelobj.check_wait(other_user) and duel.is_ai is False:
                            ...
                            if choices[0] is not None and duelobj.check_wait(user):
                                ...
                                if duel.current_priority == 0 and duel.in_trigger_waiting == 1 and duel.ask == 0 and duel.in_cost is False :
                                    duelobj.invoke_trigger_waiting(duelobj.trigger_waiting)
                                    if duel.in_cost is False:
                                        ...
                                        if duel.is_ai == True and duel.appoint == other_user:
                                            if room_number == 1:
                                                lock.lock_1 = False
                                                lock.save()
                                            elif room_number == 2:
                                        ...
                                        #duelobj.cost_result = {}
                                        #duelobj.cost = {}
                                        if duel.appoint == other_user and duel.is_ai == False  and choices2[0] is not None:
                                            flag_2 = True
                                    break
                                    ...
                                    if duel.chain == 0:
                                        ...
                                        if duel.change_appoint_flag == 0:
                                            duel.appoint = duel.user_turn
                                        duel.appoint_flag = False
                                    if duel.appoint == other_user and duelobj.check_wait(other_user) and duel.is_ai == False and choices2[0] is not None:
                                        flag_2 = True
                                    break
                if duel.appoint == user:
                    choices = duelobj.check_trigger(
                    ...
                    if choices[0] != "monster_trigger":
                        if (
                            ...
                            (choices[0] is None or choices[0] is True)
                            and (choices2[0] is None
                            or choices2[0] is True) and duel.chain == 0 and duel.in_cost is False):
                            duel.current_priority = choices[1]
                            duelobj.update = True
                            ...
                            if (
                                ...
                                duel.current_priority = duelobj.max2(choices,choices2)
                                flag2 = duelobj.invoke_trigger_waiting(duelobj.trigger_waiting, duel.current_priority)
                                duelobj.update = True
                        ...
                        if (
                            ...
                            if duel.in_cost is False and duel.current_priority == 0:
                                ...
                                if duel.chain == 0:
                                    ...
                                    #duelobj.cost_result = {}
                                    #duelobj.cost = {}
                                    duelobj.current_priority = 10000
                                    choices = duelobj.check_trigger(
                                        decks,
                                        graves,
                                        hands,
                ...
                if( choices[0] is not None
                    ...
                    break
                elif(choices[0] is None and choices2[0] is None and choices[1] is 0 and choices2[1] is 0):
                    duel.current_priority = 10000
...
def battle_det_return(
    ...
    or (duel.ask == 0 and duel.appoint == user and duel.ask2 == 0 and duel.phase.pri is True):
        if choices[0] != "force" and choices[0] is not None:
     ...
     for trigger_waiting in duelobj.trigger_waiting[:]:
         ...
         if who == 0:
             ...
             if not duelobj.check_launch_trigger(
                 ...
                 i+=1
                 #remove_waiting.append(trigger_waiting)
                 if len(duelobj.trigger_waiting) == 0:
                     duel.in_trigger_waiting = False
                 continue
             else:
