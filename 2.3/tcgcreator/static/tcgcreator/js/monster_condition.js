...
'success': function(data){
        ...
        if(kind == 57 || kind == 0 || kind == 1 || kind==2 || kind==3 || kind == 4 || kind == 5  || kind==6 || kind == 73 || kind == 81 || kind == 30 || kind == 31|| kind==35 || kind==36 || kind == 50 || kind == 25 || kind == 38 || kind == 51 || kind == 70 || kind == 71){
            $("#"+id+"_place_tab").show();
        }else{
            $("#"+id+"_place_tab").hide();
        }
                ...
                addConditionPlaceAll(id,k-1,val);
                    ...
                    $.ajax({
                        ...
                        'success': function(data){
                            $("."+id+"_field_x_and_y_0").show();
                            $("#"+id+"_field_x_and_y_0").html(data);
        if(val["monster"][0]["monster"]["field_x"]){
        for(var field_x =0;val["monster"][0]["monster"]["field_x"][field_x] != undefined;field_x++){
            if($("#"+id+"_field_x_"+0+"_"+field_x).length == 0){
		addFieldX(0,field_x,id);
	    }
            $("#"+id+"_field_x_"+0+"_"+field_x).val(val["monster"][0]["monster"]["field_x"][field_x]);

            $("#get_field_x_det_"+0+"_"+field_x).val(val["monster"][0]["monster"]["field_x_operator"][field_x]);
            $("#get_field_x_and_or_"+0+"_"+field_x).val(val["monster"][0]["monster"]["field_x_and_or"][field_x]);
        }
    }
        if(val["monster"][0]["monster"]["field_y"]){
        for(var field_y =0;val["monster"][0]["monster"]["field_y"][field_y] != undefined;field_y++){
            $("#"+id+"_field_y_"+0+"_"+field_y).val(val["monster"][0]["monster"]["field_y"][field_y]);
            $("#get_field_y_det_"+0+"_"+field_y).val(val["monster"][0]["monster"]["field_y_operator"][field_y]);
            $("#get_field_y_and_or_"+0+"_"+field_y).val(val["monster"][0]["monster"]["field_y_and_or"][field_y]);
        }
    }
        if(kind == 57 || kind == 3 || kind == 4 || kind == 1 || kind == 0 || kind == 70 || kind == 81){
...
'success': function(data){
    ...
    */
	...
	tmp+='<select id="get_monster_name_equal_'+m+'_1_'+j+'"><option value="">全て</option><option value="=">=</option><option value="!=">!=</option><<option value="like">含む</option></select><select id="monster_name_and_or_'+m+'_1_'+j+'" > <option value=""></option> <option value="and">かつ</option> <option value="or">または</option> </select><input id="monster_name_add_'+m+'_1_'+j+'" type="button" value="追加"  onclick="addMonsterName(\''+m+'_1_'+j+'\')"><br>';
        ...
        $("#step_"+String(m)).val(val["monster"][m]["step"]);
	if(val["monster"][m]["min_flag"] != undefined && val["monster"][m]["min_flag"] == true ){
		$("#min_flag_"+String(m)).prop("checked",true);
	}else{
		$("#min_flag_"+String(m)).prop("checked",false);
	}
        tmp["equation"] = {};
	...
	if(m > 0){
        ...
        $("#exclude_"+String(m)).val(val["exclude"]);
        $("#include_"+String(m)).val(val["include"]);
        $("#include_all").prop("checked",val["include_all"]);
    ...
    function putCondition(){
        ...
        if(mode == 81 || mode == 57 || mode == 3 || mode==4 || mode ==1 || mode == 9 || mode == 30 || mode == 31 || mode == 35 || mode == 36 || mode == 50 || mode == 25 ||mode==38 || mode == 51 || mode == 0 || mode == 71){
        ...
        for(var i=1;$("#get_monster_name_equal_"+m+"_"+i+"_0").length;i++){
            ...
            if($("#get_monster_name_equal_"+m+"_"+i+"_"+j).val() == ""){
                ...
                json["monster_name_kind"][j]["and_or"] =$("#monster_name_and_or_"+m+"_"+i+"_"+j).val();
                json["monster_name_kind"][j]["monster_name"] =$("#monster_name_"+m+"_"+i+"_"+j).val();
            }else if($("#get_monster_name_equal_"+m+"_"+i+"_"+j).val() == "notlike"){
                json["monster_name_kind"][j] = {};
                json["monster_name_kind"][j]["operator"] = "notlike";
                json["monster_name_kind"][j]["and_or"] =$("#monster_name_and_or_"+m+"_"+i+"_"+j).val();
                json["monster_name_kind"][j]["monster_name"] =$("#monster_name_"+m+"_"+i+"_"+j).val();
            }
        }
        ...
        json["field_x"] = [];
        json["field_x_operator"] = [];
        json["field_x_and_or"] = [];
        for(var field_x=0;$("#"+id+"_field_x_"+m+"_"+field_x).val() != undefined && $("#"+id+"_field_x_"+m+"_"+field_x).val() != "";field_x++){
            json["field_x"][field_x] = $("#"+id+"_field_x_"+m+"_"+field_x).val();
            json["field_x_operator"][field_x] = $("#get_field_x_det_"+m+"_"+field_x).val();
            json["field_x_and_or"][field_x] = $("#get_field_x_and_or_"+m+"_"+field_x).val();
        }
        json["field_y"] = [];
        json["field_y_operator"] = [];
        json["field_y_and_or"] = [];
        json["exclude"] = $("#exclude_"+String(m)).val();
        json["include"] = $("#include_"+String(m)).val();
        json["include_all"] = $("#include_all").prop("checked");
        for(var field_y=0;$("#"+id+"_field_y_"+m+"_"+field_y).val() != undefined && $("#"+id+"_field_y_"+m+"_"+field_y).val() != "";field_y++){
            json["field_y"][field_y] = $("#"+id+"_field_y_"+m+"_"+field_y).val();
            json["field_y_operator"][field_y] = $("#get_field_y_det_"+m+"_"+field_y).val();
            json["field_y_and_or"][field_y] = $("#get_field_y_and_or_"+m+"_"+field_y).val();
        }
	...
	if(mode == 81){
        	tmp["step"] = $("#step_"+m).val();
	}else{
        	tmp["step"] = 0;
	}
        tmp["min_flag"] = $("#min_flag_"+m).prop("checked")?true:false;
        tmp["equation"] = {};
       ...
       // tmp["equation"]["equation_number"] = $("#get_equation_number_"+m).val();
        ...
        tmp["and_or"] = and_or;
        val["exclude"] = $("#exclude_"+String(m)).val();
        val["include"] = $("#include_"+String(m)).val();
        val["include_all"] = $("#include_all").prop("checked");
    ...
    var j=i+1;
    $("#choose_"+id+"_"+i).after('<div id="choose_'+id+'_'+j+'"> <input type="button" class="active" value="条件1" onclick="displayCondition(\''+id+'\','+j+',1)" id="'+id+'_button_'+j+'_1"><input type="button" value="追加" id="'+id+'_add_button_'+j+'_1" onclick="addConditionPlace(\''+id+'\','+j+',1)"><br> 場所 <a class="show_place" href="javascript:showPlace()">+</a><a style="display:none" class="hide_place" href="javascript:hidePlace()">-</a> <div class="trigger_condition_place_box" style="display:none"> <select id="'+id+'_place_'+j+'_0" class="'+id+'_place" style=""> </select> <select id="'+id+'_place_add_'+j+'_0" onchange="addPlace(\''+id+'_place\','+j+',1)" class="'+id+'_place" style=""> <option value=""></option> <option value="and">かつ</option> <option value="or">または</option> </select></div> <div id="'+id+'_'+j+'_1"> </div>カード有無 <a class="show_card_exist" href="javascript:showCardExist()">+</a><a style="display:none" class="hide_card_exist" href="javascript:hideCardExist()">-</a> <div class="card_exist_box" style="display:none"> <input type="checkbox" id="monster_exist" value="1" checked> </div><br> フィールド位置  <a class="show_field_x_and_y" href="javascript:showFieldXandY()">+</a><a style="display:none" class="hide_field_x_and_y" href="javascript:hideFieldXandY()">-</a> <div style="display:none" class="field_x_and_y" id="trigger_condition_field_x_and_y_'+j+'"> </div>除外 <a class="show_exclude" href="javascript:showExclude()">+</a><a style="" class="hide_exclude" href="javascript:hideExclude()">-</a><div style="" class="exclude"> <input type="text" id="exclude_'+j+'" ></div> 含む <a class="show_include" href="javascript:showInclude()">+</a><a style="" class="hide_include" href="javascript:hideInclude()">-</a><div style="" class="include"><input type="text" id="include_'+j+'">all<input type="checkbox" id="include_all"></div><div id="'+id+'_equation_'+j+'" class="'+id+'_equation" style=""></div> </div> </div><div id="as_'+id+'_wrapper_'+j+'">as <input type="text" id="as_'+id+'_'+j+'"></div> </div > </div></div>')
            ...
            'success': function(data){
                    ...
                    $.ajax({
                        ...
                        'success': function(data){
                            ...
                            $("#"+id+"_field_x_and_y_"+j).html(data);
				/*
        for(var field_x =0;val["monster"][j]["monster"]["field_x"][field_x] != undefined;field_x++){
            if($("#"+id+"_field_x_"+j+"_"+field_x).length == 0){
		addFieldX(j,field_x,id);
	    }
            $("#"+id+"_field_x_"+j+"_"+field_x).val(val["monster"][j]["monster"]["field_x"][field_x]);
            console.log($("#"+id+"_field_x_"+j+"_"+field_x).val());
            console.log("#"+id+"_field_x_"+j+"_"+field_x);
            console.log(val["monster"][j]["monster"]["field_x"][field_x]);

            $("#get_field_x_det_"+j+"_"+field_x).val(val["monster"][j]["monster"]["field_x_operator"][field_x]);
            $("#get_field_x_and_or_"+j+"_"+field_x).val(val["monster"][j]["monster"]["field_x_and_or"][field_x]);
        }
        for(var field_y =0;val["monster"][j]["monster"]["field_y"][field_y] != undefined;field_y++){
            $("#"+id+"_field_y_"+j+"_"+field_y).val(val["monster"][j]["monster"]["field_y"][field_y]);
            $("#get_field_y_det_"+j+"_"+field_y).val(val["monster"][j]["monster"]["field_y_operator"][field_y]);
            $("#get_field_y_and_or_"+j+"_"+field_y).val(val["monster"][j]["monster"]["field_y_and_or"][field_y]);
        }
	*/
                            if(json!= null){
